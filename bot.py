...
def ask_golden_key(config: dict[str, object]) -> bool:
    expected = str(config.get("golden_key", ""))
    if not expected or expected == "CHANGE_ME":
        print("В конфиге не задан golden_key. Откройте файл и задайте значение.")
        return False
...
def run_console(config_path: Path, csv_path: Path, txt_path: Path) -> None:
    ...
    while True:
        ...
        print("3. Показать путь к конфигу")
        print("4. Выход")
        ...
        if choice == "1":
            ...
            print("Неизвестный выбор. Введите число от 1 до 4.")
